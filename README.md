...
- [ ] Backend (no fancy CLI)
	- [x] Ingestion of multiple files into database WITHOUT embedding
		- [x] For each ingestion, create a new project number and use that project number for every doc. (This is an alternative to destroying all chunks, in case I want to re-examine old chunks)
	- [x] Add embedding onto ingestion:
		- [x] Go back to `main` aka. rollback my semantic chunking shenanigans that are on the `chunking` branch.  THEY ARE OVERKILL [[LS LOA]]
		- [x] **full code review by Claude**
		- [x] look back at labs to examine how bulk embedding was accomplished: I think you feed an array of chunks, and an array of metadata?
		- [x] Implement fixed size chunking [[RAG; chunking strategies]]
		- [x] Test with LIMIT=50 - does it throw error?
	- [ ] Keyword querying of files in the DB (non-semantic, uses SQL queries)
	- [ ] 
	- [ ] Semantic querying!?
	- [ ] Add semantic search of the DB
	- [ ] Create relationship between `chunk` and `file`
- [ ] Interactive CLI
	- [ ] Initiate ingestion: 
